#include "OSX_fileIO.hpp"

using namespace gt;

cOSX_fileIO::cOSX_fileIO(){
}
...
cByteBuffer&
cOSX_fileIO::read(const dFilePoint pStartPoint, const size_t pReadAmount ){
	std::fstream stream;
...
void
cOSX_fileIO::write(const cByteBuffer* pBuff){
	std::fstream stream;
...
void
cOSX_fileIO::insert(const cByteBuffer* pBuff, dFilePoint pStartPoint){
	std::fstream stream;
