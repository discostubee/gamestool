...
namespace gt{
	...
	public:
		...
		virtual void run(cContext* pCon) =0;//{ DUMB_REF_ARG(pCon); };
	...
	protected:
		...
		virtual void read(cByteBuffer* aOutput, const dFilePoint pStartPoint = 0, const size_t pReadAmount = 0 ) =0;
			//{ DUMB_REF_ARG(pStartPoint); DUMB_REF_ARG(pReadAmount); DONT_USE_THIS;  /*cByteBuffer *temp=NULL; return *temp;*/ }

		virtual void write(const cByteBuffer* pBuff) =0;
		...
		virtual void insert(const cByteBuffer* pBuff, dFilePoint pStartSpot) =0;
